...
function CartItemSkeleton() {
    return (
        <div className="flex gap-6 py-8 border-b border-foreground/10">
            <div className="w-24 h-32 bg-dim animate-pulse rounded" />
            <div className="flex-1">
                <div className="h-3 w-20 bg-dim animate-pulse rounded mb-2" />
                <div className="h-5 w-40 bg-dim animate-pulse rounded mb-4" />
                <div className="h-4 w-24 bg-dim animate-pulse rounded" />
            </div>
        </div>
    );
}
                    ...
                    {loading ? (
                        ...
                        <div>
                            <AnimatePresence>
                                {items.map((item) => (
                                    <motion.div
                                        ...
                                        className="flex gap-6 md:gap-10 py-8 border-b border-foreground/10"
                                    >
                                        {/* Product Image */}
                                        <div className="w-24 md:w-32 aspect-[3/4] bg-dim overflow-hidden flex-shrink-0 rounded-xl border border-foreground/5">
                                            <img
                                        ...
                                        <div className="flex-1 flex flex-col justify-between">
                                            <div>
                                                <p className="text-accent text-[9px] uppercase tracking-[0.3em] mb-1">
                                                    {item.category}
                                                </p>
                                            ...
                                            <div className="flex items-center justify-between mt-4">
                                                <div className="flex items-center gap-4 border border-foreground/20 px-2 py-1">
                                                    ...
                                                    <span className="text-foreground text-sm tracking-wider w-6 text-center" aria-label={`Quantity: ${item.quantity}`}>
                                                        {item.quantity}
                                                    </span>
                                                    <button
                                                        onClick={() => handleUpdateQuantity(item.productId, item.quantity + 1, item.size)}
                            ...
                            <div className="mt-12 border-t border-foreground/20 pt-8">
                                ...
                                <Link
                                    to="/checkout"
                                    className="block w-full text-center py-4 bg-accent text-surface text-[10px] tracking-[0.3em] uppercase font-medium hover:bg-accent-hover transition-all duration-500 mb-4"
                                >
                                    Proceed to Checkout
                                </Link>
                                <div className="flex flex-col sm:flex-row gap-4">
                                    <Link
                                        to="/gallery"
                                        className="flex-1 text-center px-8 py-4 border border-foreground/20 text-foreground text-[9px] tracking-[0.5em] uppercase hover:bg-foreground hover:text-surface transition-all duration-700"
                                    >
                                        Continue Shopping
                                    </Link>
                                    <button
                                        onClick={clearCart}
                                        className="px-8 py-4 border border-foreground/10 text-subtle text-[9px] tracking-[0.3em] uppercase hover:text-red-400 hover:border-red-400/30 transition-all duration-500"
                                    >
                                        Clear Cart
                                    </button>
                                </div>
